...
//		}
	...
	wheel = function (event) {
		...
		if (options.wheelDirection === 'horizontal' && options.direction !== 'none') {
...
//		event.stopPropagation();
		...
		if (options.direction !== 'vertical' && options.direction !== 'none') {
			target.scrollLeft -= (event.pageX - position.x);
		}
		if (data.options.direction !== 'horizontal' && options.direction !== 'none') {
	...
	drift = function (target, event, callback) {
		...
		if (options.direction !== 'vertical' && options.direction !== 'none') {
			scrollLeft -= dx;
		} if (options.direction !== 'horizontal' && options.direction !== 'none') {
			scrollTop -= dy;
		}
...
//		target.css('cursor', compat.cursorGrab);
	...
	getOptions = function(options) {
		...
		if (options.direction != 'none' && options.direction !== 'multi' && options.direction !== options.wheelDirection) {
	...
	createThumbs = function(target, sizing, options) {
		...
		if (options.hasHorizontal && options.direction !== 'vertical' && options.direction !== 'none') {
		...
		else options.hasHorizontal = false;
		
		if (options.hasVertical && options.direction !== 'horizontal' && options.direction !== 'none') {
		...
		else options.hasVertical = false;
